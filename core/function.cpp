...
variable* _function::member_access(const std::string& name) const
{
	...
	{
		auto s = stropr::split_string(name, ".");
		if (s.size() < 2)
		{
			ERR(TS_ID_32);
			return nullptr;
...
//	SWITCH_CASE("if")
//	{
//		const auto x = Process(T.depends[0]);
//		if (x != nullptr)
//			if (GET_TYPE("bool", root_bool, x->get())->access()) {
//				delete x;
//				return Process(T.depends[1]);
//			}
//		delete x;
//		return Process(T.depends[2]);
//	}
//	SWITCH_CASE("while")
//	{//TODO:在循环中的return
//		variable* n = Process(T.depends[0]);
//		while (n != nullptr) {
//			delete n;
//			n = Process(T.depends[0]);
//			if (GET_TYPE("bool", root_bool, n->get())->access())
//			{
//				const auto x = Process(T.depends[1]);
//				if (x != nullptr)
//				{
//					return x;
//				}
